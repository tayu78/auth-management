                ...
                {users.map(user => {
                  return (
                    <tr key={user.id}  >
                      ...
                      <td className="sm:sticky sm:-right-5 sm:bg-white sm:border-l-2   "><EditIcon className="cursor-pointer" onClick={()=>handleEdit(user)} /><DeleteIcon onClick={() => handleDelete(user.email)} className="hover:bg-slate-300 before:border-l " /></td>
                    </tr>
                  )
                })}
              </tbody>
